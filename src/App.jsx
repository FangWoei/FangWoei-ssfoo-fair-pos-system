...
import { ROLES, signOut, watchAuth, watchProfile } from "./lib/auth";
...
function Gate() {
  ...
  if (issues.length)
    return <BadProfile uid={user.uid} profile={profile} issues={issues} />;
  ...
  if (typeof p.active !== "boolean") {
    out.push({
      ...
      found:
        p.active === undefined
          ? "missing"
          : `${t(p.active)} — ${JSON.stringify(p.active)}`,
      want: "boolean true",
  ...
  if (p.role !== "admin" && p.role !== "user") {
    out.push({
      ...
      found:
        p.role === undefined
          ? "missing"
          : `${t(p.role)} — ${JSON.stringify(p.role)}`,
    ...
    <div className="scrim" style={{ background: "var(--canvas)" }}>
      <div className="modal">
        ...
        {issues.map((i) => (
          <div className="preview" key={i.field} style={{ marginBottom: 10 }}>
            ...
            <div className="prow">
              ...
              <span className="mono" style={{ color: "var(--teal-lit)" }}>
                {i.want}
              </span>
            </div>
            <p
        ...
        <pre
          ...
          {JSON.stringify(
            Object.fromEntries(
              Object.entries(profile).filter(([k]) => k !== "uid"),
            ),
            null,
            2,
          )}
  ...
  return (
    <ToastCtx.Provider value={notify}>
      <div className="app">
        ...
        {view === "report" && <Report till={till} products={products} />}
      ...
      <div className="toasts">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={t.tone === "warn" ? "toast warn" : "toast"}>
            {t.message}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="scrim" style={{ background: "var(--canvas)" }}>
      <div className="modal">
        ...
        <p className="lede">
          The login works, but nobody has said who you are or what you're
          allowed to do. An admin creates that under Staff. If you're setting up
          the first admin, do it once by hand:
        </p>
        ...
        <ol
          ...
          <li>
            Start a collection called{" "}
            <b style={{ color: "var(--text)" }}>users</b>
          </li>
...
class Boundary extends Component {
  ...
  render() {
    if (this.state.err)
      return <Problem message={this.state.err.message} crash />;
