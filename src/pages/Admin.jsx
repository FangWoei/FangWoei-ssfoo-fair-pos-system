...
import { saveProduct, newProductRef, removeProduct } from '../lib/db';
  ...
  return (
    <div className="page">
      <div className="page-head">
        ...
        <button className="btn primary" style={{ flex: '0 0 auto', padding: '0 20px' }}
            ...
            {products.map((p) => (
              <tr key={p.id}>
                ...
                <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                  <button className="linkbtn" style={{ color: 'var(--text-dim)' }}
...
function describe(offer) {
  if (offer.type === OFFER_FREEBIE) return `Buy ${offer.buyQty} free ${offer.freeQty}`;
  if (offer.type === OFFER_BULK)
    return offer.tiers.map((t) => `${t.qty} for ${formatRM(t.price)}`).join(', ');
  return 'None';
}
...
function ProductForm({ initial, isNew, onSave, onDelete, onClose }) {
  ...
  const setOffer = (k, v) => setF((s) => ({ ...s, offer: { ...s.offer, [k]: v } }));
  ...
  const addTier = () =>
    setOffer('tiers', [...f.offer.tiers, { qty: (f.offer.tiers.at(-1)?.qty || 1) + 1, price: 0 }]);

  const dropTier = (i) => setOffer('tiers', f.offer.tiers.filter((_, x) => x !== i));
    ...
    const offer =
      f.offer.type === OFFER_BULK
        ? {
            type: OFFER_BULK,
            tiers: f.offer.tiers
              .map((t) => ({ qty: Number(t.qty) || 0, price: toSen(t.priceInput ?? toRMInput(t.price)) }))
  ...
  return (
    <div className="scrim" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal wide">
        ...
        <label className="field">
          <span>Offer</span>
          <select value={f.offer.type} onChange={(e) => setOffer('type', e.target.value)}>
          ...
          <div style={{ marginBottom: 12 }}>
            {f.offer.tiers.map((t, i) => (
              <div className="tierrow" key={i}>
                ...
                <button className="linkbtn danger" onClick={() => dropTier(i)}
                  style={{ marginTop: 18 }}>
                  Remove
                </button>
              </div>
        ...
        <div className="preview mono">
          ...
          {previewQtys.map((q) => {
            const r = priceLine({ unitPrice: price, qty: q, offer: previewOffer });
            return (
        ...
        <label style={{ display: 'flex', gap: 9, alignItems: 'center', margin: '14px 0' }}>
          <input
            type="checkbox"
...
const dedupe = (a) => [...new Set(a.filter((n) => n > 0))].sort((x, y) => x - y);
...
const SAMPLE = (() => {
  ...
  const free = (buyQty, freeQty) => ({ type: OFFER_FREEBIE, buyQty, freeQty, tiers: [] });
